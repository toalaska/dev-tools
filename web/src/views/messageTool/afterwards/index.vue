  ...
  <el-row :gutter="10" class="home">
    ...
    <el-col v-if="data.length > 0" :span="19" class="info--wrapper">
      ...
      <div v-for="item in data" :key="item.judgeResultId" class="info--body">
        ...
        <div
          v-for="items in item.settlements"
          :key="items.settlementDisease.diseaseCode"
          class="info--wrapper"
        >
          <div class="info--header" style="margin-left:20px">
            <span style="flex: 2;">diseaseCode</span>
            <span style="flex: 2;">diseaseName</span>
          </div>
          <div class="row parent" style="margin-left:20px">
            <span style="flex: 2;">{{ items.settlementDisease.diseaseCode }}</span>
            <span style="flex: 2;">{{ items.settlementDisease.diseaseName }}</span>
          </div>
          ...
          <div
            v-for="(child, index) in items.inpatientCostDetails"
  ...
  methods: {
    ...
    getAmount() {
      ...
      JSON.parse(this.params.json).bizDatas.forEach((item) => {
        item.settlements.forEach((items) => {
          items.inpatientCostDetails.forEach((inpatientCostDetails) => {
            const arr = this.projectNo.split(",");
            arr.forEach((item) => {
              if (item === inpatientCostDetails.projectNo) {
                this.amount += inpatientCostDetails.amount;
                return inpatientCostDetails;
              }
            });
          });
        });
